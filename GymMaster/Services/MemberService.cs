...
namespace Services
{
    public class MemberService:IMemberService
    {
        ...
        {
            return await _repository.GetAll().Include(m=>m.Memberships).CountAsync(m=>m.Memberships!.Any(ms=>ms.EndDate>DateTime.Now));
        }
        public async Task<IEnumerable<Member>> GetExpiredMembers()
        {
            return await _repository.GetAll().Include(m=>m.Memberships).Where(m=>m.Memberships!.Any(ms=>ms.EndDate<DateTime.Now)).ToListAsync();
        }
        public async Task<int> GetInactiveMembers()
        {
            return await _repository.GetAll().Include(m=>m.Checkins).CountAsync(m=>!m.Checkins!.Any(c=>c.CreatedAt >= DateTime.Today.AddMonths(-1)));
        }
        ...
        {
            var exisingMemeber = await _repository.GetSingleAsync(id);
            if(exisingMemeber is null)
            {
                return null;
            }
            exisingMemeber.UpdateAt = DateTime.Now;
            exisingMemeber.Notes = member.Notes;
            //exisingMemeber.Notes = member.Notes;
            exisingMemeber.Sex = member.Sex;
            exisingMemeber.Address  = member.Address;
            exisingMemeber.TrainerId = member.TrainerId;
            
            return await _repository.UpdateAsync(exisingMemeber);
        }
